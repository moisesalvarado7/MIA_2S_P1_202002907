...
type EBR struct {
	Part_status [100]byte //Sustituye a Part_mount
	Part_fit    [100]byte
	Part_start  [100]byte
	Part_size   [100]byte
	Part_next   [100]byte
...
func Analizar() {
	fmt.Println("API funcionando en el puerto 5000")
...
func graficar_disk(direccion string, destino string) {
	...
	if master_boot_record.Mbr_tamano != empty {
		if err == nil {
			...
			salida_comando += "[SUCCES] Reporte generado con exito!\\n"
			if err != nil {
				salida_comando += "[ERROR] Error al escribir en el archivo\\n"
				return
			}
